...
linux: web
	...
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o dist/sub2api-limit-updater-linux-amd64 ./cmd/sub2api-limit-updater
	...
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o dist/sub2api-limit-updater-linux-arm64 ./cmd/sub2api-limit-updater
