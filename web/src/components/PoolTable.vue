    ...
    <table class="data-table">
      ...
      <TransitionGroup tag="tbody" name="row">
        <tr v-for="account in filtered" :key="account.id">
          ...
          <td data-label="5h 窗口"><QuotaBand label="5h" kind="pool" accent="mint" dense :window="account.window_5h || account.usage_5h" /></td>
          <td data-label="7d 窗口"><QuotaBand label="7d" kind="pool" accent="violet" dense :window="account.window_7d || account.usage_7d" /></td>
          <td data-label="数据时间">
            <span class="timestamp"><Clock3 :size="13" />{{ formatDateTime(account.snapshot?.source_updated_at || account.snapshot?.as_of) }}</span>
          </td>
        </tr>
      </TransitionGroup>
    </table>
