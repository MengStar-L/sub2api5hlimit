...
<template>
  <AppShell>
    <header class="page-heading">
      <div><span class="eyebrow"><Download :size="13" /> 受控发布</span><h1>程序更新</h1><p>检查 GitHub 稳定版，并在校验成功后安全重启门户。</p></div>
      <div class="heading-actions"><button class="icon-button" type="button" title="刷新状态" :disabled="loading" @click="load()"><RefreshCw :size="16" :class="{ spinning: loading }" /></button><button class="secondary-button" type="button" :disabled="checking || controlsBlocked" @click="check"><LoaderCircle v-if="checking" :size="15" class="spinning" /><RefreshCw v-else :size="15" />检查更新</button></div>
    </header>

    <section v-if="loading && !update" class="update-loading"><span class="skeleton skeleton-row"></span><span class="skeleton skeleton-row"></span></section>
    <template v-else-if="update">
      <section class="update-overview">
        <article><span>当前版本</span><strong class="mono">{{ update.current.version }}</strong><small><ServerCog :size="12" />{{ update.current.os || '—' }} / {{ update.current.arch || '—' }}</small></article>
        <article><span>最新稳定版</span><strong class="mono">{{ update.latest?.version || '尚未发现' }}</strong><a v-if="update.latest?.release_url" :href="update.latest.release_url" target="_blank" rel="noreferrer">查看 Release <ExternalLink :size="12" /></a><small v-else>请先检查更新</small></article>
      ...
      <section class="panel accent-cobalt update-detail">
        <div class="panel-head"><div><h2>更新兼容性</h2><p>仅安装与当前部署兼容的二进制更新包。</p></div><div class="panel-tools"><a v-if="update.latest?.release_url" class="text-button" :href="update.latest.release_url" target="_blank" rel="noreferrer">Release <ExternalLink :size="13" /></a></div></div>
      ...
      <section v-if="operation || reconnecting" class="panel update-operation" :class="operation?.rolled_back || operationMismatch || recoveryRequired ? 'accent-amber' : operation?.state === 'succeeded' ? 'accent-mint' : 'accent-cobalt'">
        <div class="panel-head"><div><span class="section-icon" :class="operation?.rolled_back || operationMismatch || recoveryRequired ? 'amber' : operation?.state === 'succeeded' ? 'mint' : 'cobalt'"><RotateCcw v-if="operation?.rolled_back" :size="16" /><CircleAlert v-else-if="recoveryRequired" :size="16" /><Check v-else-if="operation?.state === 'succeeded'" :size="16" /><LoaderCircle v-else :size="16" :class="{ spinning: active || reconnecting }" /></span><div><h2>{{ reconnecting ? '正在等待服务恢复' : phaseLabel || '最近一次更新' }}</h2><p>{{ reconnecting ? '浏览器将自动重新连接，并核对实际运行版本。' : operationResult || `目标版本：${operation?.target_version}` }}</p></div></div></div>
        <div class="panel-body">
          ...
          <div v-if="operation?.rolled_back || operationMismatch || recoveryRequired || (operation && !active && operation.error_code)" class="notice tone-danger"><CircleAlert :size="16" /><span>{{ operationResult || '更新没有完成。' }} 可运行 <code>journalctl -u sub2api-limit-portal-update.service -n 100</code> 查看系统日志。</span></div>
        </div>
      </section>
    </template>
